...
//	in this mode every record can have a free structure of attributes, only 'loc' is required
	...
	onAdd: function (map) {
		...
		// 		'layerremove': this._onLayerAddRemove
			 ...
			 'zoomstart': function(){ this.collapse(); },
			 'popupopen': function(){ this.collapse(); }
		 	}, this);

			var that = this;
			$(document).on('click', '*', function(e) { if (e.target.className == 'search-input') return; that.collapse(); });
			$('div#sidebar').on('mouseover', function() { that.collapse(); });
	...
	_createTooltip: function(className) {
		...
		L.DomEvent
			...
			.on(tool, 'mouseover', function(e) {
				that.collapseDelayedStop();
				$('.search-tip-select').removeClass('search-tip-select');
				this._tooltip.currentSelection = 0;
			}, this);
		return tool;
	},

	_createTip: function(text, val, key) {//val is object in recordCache, usually is Latlng
		var tip;
		...
		else
		{
			...
			tip.href = '#';
			tip.setAttribute('data-key', key);
			tip.innerHTML = text.replace(/^(.*?) - /, '<strong>$1</strong> - ');
		}
...
// fix removal of duplicates
			newTip = this._createTip(filteredRecords[key].title, filteredRecords[key], key);
...
//			this._input.value = searchTips[this._tooltip.currentSelection]._text;
	...
	_handleSubmit: function(key) {	//button and tooltip click and enter submit

		var id = key || this._tooltip.currentSelection;
...
//		this._hideTooltip();
	...
	_getLocation: function(key) {	//extract latlng from _recordsCache
		if( this._recordsCache.hasOwnProperty(key) )
			return this._recordsCache[key].loc;//then after use .loc attribute <--??? edit: fixed removal of duplicates
		else
			return false;
	},
